...
import NewCategorie from "../../components/Forms/NewCategorie";

export default function AdminCategories() {
  const { loading, setLoading, peticionAllCategories, categories, peticionDeleteCategorie } =
    useContext(Context);

  const [pagina, setPagina] = useState(1);
  const [porPagina, setPorPagina] = useState(10);

  const maximo = Math.ceil(categories.length / porPagina);
  ...
  const titles = ["ID", "Nombre", "Opciones"];
                              ...
                              <td className="whitespace-nowrap  px-6 py-4 font-medium">
                                <div className="inline-block">
                                  <TERipple rippleColor="white">
                                    <button
                                      type="button"
                                      onClick={(e) => peticionDeleteCategorie(item.id_categorie)}
