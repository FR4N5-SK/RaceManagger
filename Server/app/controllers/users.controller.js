...
class UsersController {
  ...
  async login(req, res) {
    try {
      ...
      const token = jwt.sign({ id: user.id_user, usernmae: username, role: user.role }, process.env.JWT_SECRET, { expiresIn: '1h' });

      res
        .status(200)
        .json({ status: 200, message: "Inicio de sesión exitoso.", result: {
  ...
  async repairPassword(req, res) {
    try {
      ...
      const userUpdate = await usersModel.updatePasswordById(username, hashedPassword);
